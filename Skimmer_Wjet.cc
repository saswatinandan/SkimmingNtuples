...
void Skimmer::Loop(TString outputName, bool Muon, int skm)
{
  ...
  for (int jentry=0; jentry<nentries;jentry++) {
    ...
    for  (int itau=0 ; itau < nTau; itau++) {
      ...
      bool antielemu = (Muon) ? (tauByMVA6LooseElectronRejection->at(itau) !=0 && tauByTightMuonRejection3->at(itau) !=0) : (tauByMVA6TightElectronRejection->at(itau) !=0 && tauByLooseMuonRejection3->at(itau) !=0);
